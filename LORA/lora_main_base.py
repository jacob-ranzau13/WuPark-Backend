...
PACKET_PAYLOAD = 222  # 240 max payload - 6 bytes routing header - 12 bytes data header
"""
...
custom header layout (12 bytes):
    [0:2]  packet number   (uint16, big-endian)
    [2:4]  checksum        (uint16, sum of data bytes & 0xFFFF)
    [4:6]  total packets   (uint16, big-endian)
    [6:7]  node id          (uint16, big-endian, optional for tracking)
    [7:8]  node status     (uint8, e.g. 0 for normal, 1 for warning, 2 for error)
    [8:12] timestamp       (uint32, seconds since epoch, optional for tracking)
"""
...
class sender_node():
    ...
    def send_file(self, dest_addr: int, filepath: str, lot_id: int = 0, node_status: int = 0):
        ...
        for packet_num, chunk in enumerate(chunks):
            ...
            print(f"Sending packet {packet_num + 1}/{total_packets}")
            time.sleep(5)               # brief gap between packets (was 0.1)
    ...
    def send_bytes(self, dest_addr: int, payload: bytearray, dest_freq: int = 868):
        ...
        # construct routing header
        # receive() expects to strip source address and frequency from the first 6 bytes, so we put them at the start of the header
        header = bytes([
            ...
            self.node.addr >> 8,     # source address high byte
            self.node.addr & 0xFF,   # source address low byte
...
class receiver_node():
    ...
    def receive_file(self, output_path: str):
        ...
        while total_packets is None or len(packets) < total_packets:
            ...
            # routing header: [src_addr(2), src_freq(1), dst_addr(2), dst_freq(1)] = 6 bytes
            # but receive() strips src_addr and src_freq, so routing header is effectively just [dst_addr(2), dst_freq(1)] at the start of the payload, 3 bytes total
            # custom header:  [packet_num(2), checksum(2), total_packets(2), reserved(6)] = 12 bytes

            if len(raw) < 15:  # 3 bytes stripped routing header + 12 bytes custom header minimum
            ...
            if checksum != expected_checksum:
                print(f"Checksum mismatch on packet {packet_num + 1}, discarding.")
                continue

            packets[packet_num] = chunk
            print(f"Received packet {packet_num + 1}/{total_packets} from node {node_id}")
        ...
        return (node_id, node_status, timestamp, output_path)
